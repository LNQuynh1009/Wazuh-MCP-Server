...
@mcp.tool()
def check_alert_iocs(alert_json: str, cache: dict = None):
    ...
    Args:
        ...
        cache: Optional dictionary to use for caching results
    ...
    try:
        ...
        for ip in list(public_ips)[:5]:
            ...
            if cache is not None and ip in cache:
                ...
                # Adapt cached format to current format if needed, or just use it
                # Assuming cache stores the combined result
                
                combined_verdict = cached_result.get("verdict", "CLEAN")
            ...
            results["ips_checked"].append(merged_result)
            results["summary"]["public_ips_checked"] += 1
            
            # Update cache if provided
            if cache is not None:
                cache[ip] = merged_result
